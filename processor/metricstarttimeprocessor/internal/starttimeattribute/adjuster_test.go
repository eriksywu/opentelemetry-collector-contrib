...
func (m *mockPodClient) GetPodStartTime(ctx context.Context, podID podIdentifier) (time.Time, error) {
	key := podID.Value
	if t, ok := m.startTimes[key]; ok {
		return t, nil
	}
	return time.Time{}, fmt.Errorf("pod not found: %s", key)
}
...
func TestAdjustMetrics(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			metrics := createTestMetrics(tt.attrs, tt.isCumulative)
...
func createTestMetrics(attrs map[string]string, cumulative bool) pmetric.Metrics {
